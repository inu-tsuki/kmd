...
6. **`underwater`**（组合预设，非新 shader）：`fn` 内组合 `DisplaceFilter`（波纹）+ `DuotoneFilter`（蓝移 tint，shadow `#0a1e3f` / highlight `#5fb8d6`）+ Pixi `BlurFilter`（轻 blur，strength=1），串进 `target.filters`，**一个 ticker/addModifier 驱动全部**。
