...
> 最近更新：2026-07-09（M2 氛围集落地：vignette + scanline + noise + dissolve，见 §0.5.2；Task B 审查修复 + 语法方向，见 §0.5/§0.5.1）
> 代号：DIP-FX
...
12. **删除死路径集群**：审计确认 legacy `KineticText.play`/`bakeTimeline`/`skipToEnd`/`applyParagraphEffects` + `TextPlayer` legacy `play`（setTimeout 驱动）/`executePerformance`/`sliceLegacyPlaybackAssembly`/`fastForward`/`bakeTimeline`/`skipToEnd`/`isNewLineItem` + `LegacyTextPlaybackOptions` 接口 + orphan `example/layout-debug.ts` 在 Segment 引擎**零引用**（生产路径 `ReaderRuntimeSession → scriptPlayer.playSegment → SegmentBuilder → TextPlayer.buildTimeline` 完全不触碰），已全部删除。`LayoutEngine.ts` 注释行一并清理。保留 `TextPlayer.buildTimeline`/`captureTween`/`captureEntrance`（Segment 引擎核心）。`docs/archive/` 历史快照不动（archive 语义允许 stale）。`_pendingGlobalEffects` 字段已于主题二 S3 删除（2026-08-10，commit `b8e8815`：声明/接口字段/写入三点清除，全仓确认零消费者——真实消费链是 `SegmentBuilder` 直读 `pData.globalEffects`；台账见 `docs/planning/theme-2-yard-sweep-2026-08.md`）。
...
1. **B1 `bg(color)`**：stage 命令 `bg` 注册在 `stagePresets.ts`，`@ bg(color=...)` 委托 `stageManager.setBackgroundColor`。与 `visual.ts` 的元素级 `bg` 样式（Graphics 画圆角矩形，已改名 `box`）是不同概念——stage bg 设画布底色，element bg 画元素背景。**注意**：`bg(...)` 需 `@` 前缀——裸 stage 命令不被 parser 自动识别（同 `cam.*`，`@ cam.move(...)` 也要 `@`）。未来 Phase B 链语法统一时迁移到主语形态 `@ bg.set(...)`。
...
### 0.5.1.1 bg 参数透传修复（SA-39，2026-07-09）

`StageRuntime.apply` 原逻辑对所有字符串参数调 `resolveNumeric`，若 `parseFloat` 得 `NaN` 则返回 fallback `0`。`bg(src="tests/assets/sample-bg.jpg")` 的 `src` 参数是 URL 字符串 → `parseFloat` 得 `NaN` → fallback `0` → `stagePresets["bg"]` 内 `src.startsWith("http")` 崩溃（`TypeError: src.startsWith is not a function`）。同理 `bg(color="#1a0a2e")` 的 hex 色值也会被吞成 `0`。

**修法**：`StageRuntime.apply` 参数解析改为三路——先尝试 `resolveReference`（var/marker 引用），再 `parseFloat`（数字字符串），两者都不命中则保留原始字符串透传。数值字符串仍经 `resolveNumeric` 解析为数字（回归保护见 SA-39 断言 4）。

**语法修正**（同批）：测试样例 `fx-bg.kmd` / `fx-cyberpunk-title.kmd` 中的裸 `bg(...)` 改为 `@ bg(...)`（裸 stage 命令不会被 parser 识别为指令——parser 只在 `@` 后才走 `parseCommandChains`；未来 `bg` 会演进为 `@ bg.set` 主语形态）。`fx-cyberpunk-title.kmd` 镜头2/5 的多 `[]` 块合并为单 `[]` 内空格分隔（一段只能有一个 `[]`，后续 `[]` 被静默丢弃）。

门禁：`pnpm build` + `pnpm test:parser` + `pnpm test:playback` (277，新增 [25] SA-39 6 个断言) + `pnpm test:invariants` ✅。

### 0.5.1.2 bg(color) 清除 bg(src) 异步加载的 epoch 守卫（SA-40，2026-07-09）

**现象**：`@ bg(src="tests/assets/sample-bg.jpg")` 后接 `@ bg(color="#1a0a2e")`，纯色背景被图片"重新出现"覆盖——`bg(color)` 的清除被异步加载的 resolve 静默撤销。

**根因**：`bg(src)` 启动 `Assets.load(url).then(...)`（异步 fire-and-forget），`loadBackgroundFromUrl` 返回 epoch N。`bg(color)` 同步调 `setBackgroundSprite(null)` 清除 sprite，但**不推进 epoch**。异步 resolve 到达时 `currentBgEpoch === epoch N`（仍匹配）→ sprite 被重新挂上 → `bg(color)` 的清除被撤销。

**修法**：`setBackgroundSprite(null)` 推进 `_bgEpoch`，使待 resolve 的异步加载因 epoch 不匹配而丢弃。这与 Bug 7（并发 `bg(src)` 纪元守卫）同源——原 Bug 7 只处理"两个 `bg(src)` 竞态"，遗漏了"`bg(color)` 清除 `bg(src)` 异步加载"这一方向。

门禁：`pnpm build` + `pnpm test:parser` + `pnpm test:playback` (282，新增 [26] SA-40 5 个断言) + `pnpm test:invariants` ✅。

### 0.5.1.3 bg 命令延迟执行修复（SA-41，2026-07-09）

**现象**：脚本开始播放后立刻生效最后一条 `bg(color)`，从一开始就覆盖所有 `bg(color)` 和 `bg(src)`，停止播放后不清除。

**根因**：`buildStageModifierRecord` 对 `bg` 返回 `null` → `applyStageConfigs` 走 line 878 同步 `stageManager.apply(config.name, config.params)` 路径——所有 `bg` 命令在 **build 期** 立刻按数组顺序执行，最后一条赢。`bg` 返回 `void`（无 tween），`captureTween` 无内容捕获 → 没有时间线位置追踪，也没有记入 `stageModifierRecords` → seek 时 `replayStageModifiers` 不重放 `bg` → stop 时 `loadState(entryCheckpoint)` 恢复的是 build 前的初始状态（但因 `bg` 已同步修改了 stage 状态，`entryCheckpoint` 在 `applyStageConfigs` 之前捕获，理论上应恢复初始——但异步 `Assets.load` 的 resolve 在 stop 后到达会重新挂上 sprite）。

**修法**：`buildStageModifierRecord` 对 `bg` 返回 record fragment（`{ command, params }`，无 `isClearBoundary`、无 `duration`）。`bg` 进入 `applyStageConfigs` 的 `stageRecord && !isClearBoundary` 分支（line 810），走 `segmentTl.call(...)` 延迟执行（在 cursor 位置触发，而非 build 期同步执行），并记入 `stageModifierRecords` 供 `replayStageModifiers` seek 重放。与 `cam.shake`/`cam.drift` 的 modifierBased 延迟路径同构，但 `bg` 无 modifier 语义（不注册 modifier、无 duration），纯粹是"在时间线位置设置背景状态"。

门禁：`pnpm build` + `pnpm test:parser` + `pnpm test:playback` (290，新增 [27] SA-41 8 个断言) + `pnpm test:invariants` ✅。

### 0.5.1.4 bg(src) 同 URL 纹理卸载与 stop 取消 pending load（SA-42/SA-43，2026-07-10）

**现象**：`fx-bg.kmd` 播放到 L25 `@ bg(color="#0f3460", src="tests/assets/sample-bg.jpg")` 后，画布只剩 `#0f3460`，图片背景消失；停止和重播后状态不稳定。刷新页面并去掉 L25 后，播放到 L32 `@ bg(src="tests/assets/sample-bg.jpg")` 时复现为 `#1a0a2e`（L12 的背景色）。

**根因一（SA-42）**：L25/L32 都是连续加载同一个 `sample-bg.jpg`。`Assets.load(sameUrl)` 会复用缓存 `Texture`；旧实现 `setBackgroundSprite(newSprite, sameUrl)` 先销毁旧 sprite，再无条件 `Assets.unload(oldUrl)`。当新旧 URL 相同，新 sprite 仍在使用的缓存纹理被卸载，背景图退化为不可见，只剩 renderer clear color。因此 L25 剩 `#0f3460`；去掉 L25 后，L32 剩此前 L12 设置的 `#1a0a2e`。

**根因二（SA-43）**：`stop()` 通过 `loadState(entryCheckpoint.stage)` 恢复入口状态。若 `bg(src)` 已启动但图片尚未 resolve，`_bgSprite` 仍可能是 `null`；旧 `loadState` 只有当前存在 sprite 时才 `setBackgroundSprite(null)`，不会推进 `_bgEpoch`，pending resolve 仍可能在 stop 后挂回背景。

**浏览器 e2e 补充根因（SA-44/SA-45）**：build 期背景图片尚未 resolve 时，`:bg` effect record 保存了 `paragraphText` fallback；seek replay 会把滤镜错误应用到文字容器，触发 Pixi `FilterPipe` 的 `alphaMode null` 异常。即使 replay 时重新解析 live sprite，旧顺序仍会先给上一张 sprite 挂滤镜，再由 `bg` replay 销毁它。

**修法**：

- 背景纹理卸载延迟到下一 tick；`loadBackgroundFromUrl(url)` 与 `setBackgroundSprite(newSprite, url)` 都会取消同 URL 的 pending unload。`bg(color, src)` 已知将立即复用当前同 URL 时，清屏显式传 `unloadTexture:false`，避免真实 replay 的异步边界超过单个 tick；真正保持无图或切到其他 URL 时仍会释放旧纹理。
- 恢复到 `bgSpriteUrl=null` 的 checkpoint 时无条件 `setBackgroundSprite(null)`，即使当前没有 sprite，也推进 `_bgEpoch` 并清掉等待中的 `onBackgroundReady` 回调。
- `bg(color, src)` 先 `setBackgroundColor(color)` + `setBackgroundSprite(null)`，再 `loadBackgroundFromUrl(url)`，使“图片加载前显示纯色 fallback”的样例语义真实成立，并防止旧图/旧 `:bg` filter 延续到新图。
- behavior/instant record 显式保存 `targetLevel: "bg"`，seek replay 当下重新解析背景 sprite；若图片未就绪则通过 `onBackgroundReady` 延后 apply，绝不使用 build-time 文字 fallback。
- seek 先 `replayStageModifiers` 恢复当前背景，再 replay behavior/instant filters，确保滤镜只挂到本轮有效 sprite。

验证：真实 Chromium + Pixi WebGL 定点 seek L25/L32 后，`sample-bg.jpg` 均可见，sprite/texture/source 均为 `destroyed=false`，页面无异常。自动门禁见本轮最终验证结果。

赛博朋克 demo 方向优先做 scanline/noise（CRT/数字降解核心），vignette（暗角），dissolve（消散转场）。

4 个滤镜按 §2 通用契约交付：

1. **`vignette`**（instant）：径向亮度衰减 `smoothstep(uRadius, uRadius-uSoftness, dist(uv,0.5))`。uniform `uRadius`(0.75) / `uSoftness`(0.45)。预乘 alpha 对偶。无 padding。`mutexGroup: "filter_vignette"`, `stackable: true`。推荐 `:block`。

2. **`scanline`**（behavior）：CRT 周期亮度调制 `sin(uv.y * uDensity * uInputSize.y + uTime)` + 可选桶形畸变 `uCurvature` + 闪烁 `uFlicker`。uniform `uTime`(0) / `uDensity`(2) / `uCurvature`(0) / `uFlicker`(0)。char 级 `addModifier` + 容器级 `gsap.ticker.add`（模板同 `rgbShift`/`blur`）。无 padding。`mutexGroup: "filter_scanline"`。推荐 `:block`。

3. **`noise`**（behavior）：时变噪声叠加，文件作用域 `hash21(vec2)`（GLSL ES 3.00 禁止嵌套函数定义）。`uMono` 控制单色/彩噪。uniform `uTime`(0) / `uAmount`(0.1) / `uMono`(1) / `uScale`(4)。同 scanline 驱动模式。无 padding。`mutexGroup: "filter_noise"`。

4. **`dissolve`**（behavior）：噪声场与 `uProgress` 阈值比较——低于阈值 `alpha=0`（消散），边缘带用 `uEdge` 上色。文件作用域 `hash21`。uniform `uProgress`(0) / `uScale`(8) / `uEdge`(vec3, `hexToVec3`)。需 padding `ceil(scale)`。**progress 来源**（§7.2 定案）：`const state = { progress: 0 }` + `gsap.to(state, {progress:1, duration, ease})`（同构 `fadeShake`）。char 级 `addModifier` 读 `state.progress` + `return tween`（bare tween → `BehaviorCleanup.tween`）；容器级 `gsap.ticker.add` 读 `state.progress` + `return { filters, tickerFn, tween }`。作者 `progress=` 静态给定则锁定（不启动 tween）。`mutexGroup: "filter_dissolve"`。

**`unpackBehaviorResult` 扩展**（单点修改，INV-7/SA-16）：`BehaviorFilterResult` 加可选 `tween?: gsap.core.Tween | gsap.core.Timeline` 字段。`unpackBehaviorResult` 的 `BehaviorFilterResult` 分支补 `tween = bfr.tween`。`BehaviorCleanup.tween` 类型从 `gsap.core.Tween` 扩展为 `gsap.core.Tween | gsap.core.Timeline`。`clearBehaviors` 已 kill tween（line 297-298），无需改 cleanup 逻辑。三处调用点共用同一 `unpackBehaviorResult`，自动同步。

**GLSL 纪律**：`hash21` 函数定义在文件作用域（`void main()` 之前），同 halftone/bloom 的 `luma()` 模式。`pnpm test:shaders` 验证 4 个新 shader 全部编译通过（18 passed, 0 failed）。

门禁：`pnpm build` + `pnpm test:parser` + `pnpm test:playback` (260) + `pnpm test:invariants` + `pnpm test:shaders` (18) ✅。示例：`fx-vignette.kmd` / `fx-scanline.kmd` / `fx-noise.kmd` / `fx-dissolve.kmd`。

### 0.5.3 M2 收尾落地（displace + underwater + warp 容器级扩展，2026-07-09）

M2 旗舰镜头收尾：`displace`（波纹位移）+ `underwater`（组合预设）+ `warp` 从 char-only 扩展到容器级。

5. **`displace`**（behavior）：噪声场驱动 UV 位移 `vTextureCoord += noise(uv*uScale + uTime) * uAmount * uInputSize.zw`。噪声用 `sin` 双轴组合 `clamp` 到 `[-1,1]` 中心对称生成连续平滑位移场（避免 `hash21` 白噪点的撕裂 + 避免 `*2-1` 的固定偏移；spec §7.2 原文 `noise()` 无强制实现）。**`uAmount` 是像素值**（10 = 10px 最大位移，非 UV 比例），`uInputSize.zw`（1/纹理宽高）把像素值转成 UV 偏移量。uniform `uTime`(0) / `uAmount`(10) / `uScale`(4) + `uInputSize`（Pixi 注入）。纯坐标位移，pass-through color（同 WarpFilter，不做预乘 alpha 对偶）。需 padding（位移越界）——ctor + amount setter 同步 `Math.ceil(amount)`（像素值直接对应 padding 单位）。char 级 `addModifier` + 容器级 `gsap.ticker.add`（模板同 scanline/noise）。`mutexGroup: "filter_displace"`。

6. **`underwater`**（组合预设，非新 shader）：`fn` 内组合 `DisplaceFilter`（波纹）+ 当前 surface 的 duotone profile（蓝移 tint，shadow `#0a1e3f` / highlight `#5fb8d6`）+ Pixi `BlurFilter`（轻 blur，strength=1），串进 `target.filters`，**一个 ticker/addModifier 驱动全部**。
   - **首个返回 `filters: Filter[]` 的 preset**——此前 `clearBehaviors` 的 `Array.isArray` 分支（line 315）只有代码阅读覆盖、无真实 preset 触发。本 PR 加 `test:playback` [20.5] 用例验证：build 后 kt.filters 恰好 3 个，多次 seek 来回幂等不堆积（每次 clearBehaviors 移除全部 3 个再重 apply 3 个）。
   - **duotone 按 surface 直接构造 `TextDuotoneFilter` / `BackgroundDuotoneFilter`**（不调 duotone effect fn，避免双注册 + 双 push）。
   - char 级 `return { filters: [...] }`（无 tickerFn，addModifier 驱动 displace.time；同 dissolve char 级 `{ filters, tween }` 形态，unpackBehaviorResult 经 `'filters' in result` 分支捕获数组供 cleanup）。
   - 容器级 `return { filters: [...], tickerFn }`。
   - cleanup 由 `clearBehaviors` 的 `Array.isArray` 分支 + `destroyFilterDeep`（BlurFilter 子 pass blurX/blurY 已覆盖）处理，**无需改 PlaybackController**。
   - `mutexGroup: "filter_underwater"`（组合级互斥）。

7. **`warp` 容器级扩展**：原 char-only（`targetType:"char"`，对容器 warn no-op）。改为 `targetType:"both"`：去掉 `instanceof KineticChar` guard，char 级保持 `addModifier` + `return filter`，容器级加 `gsap.ticker.add` + `return { filters, tickerFn }`（与 scanline/noise 完全同构）。WarpFilter 的非标准 `warpUniforms` resource key 不改（保持现有 filter 类）。此前 `[.warp:block]` 会 warn no-op，扩展后生效。

**GLSL 纪律**：DisplaceFilter 的 `displaceNoise` 函数定义在文件作用域（GLSL ES 3.00 禁嵌套函数定义，同 hash21/luma 模式）。`pnpm test:shaders` 验证 DisplaceFilter shader 编译通过（19 passed, 0 failed，18 files）。

门禁：`pnpm build` + `pnpm test:parser` + `pnpm test:playback` (271，新增 [20.5] 11 个断言覆盖 displace/warp 容器级/underwater Filter[] seek 幂等) + `pnpm test:invariants` + `pnpm test:shaders` (19) ✅。示例：`fx-displace.kmd` / `fx-underwater.kmd` / `fx-cyberpunk-title.kmd`（赛博朋克标题序列 demo——scanline+noise+rgbShift+warp+dissolve+vignette+underwater+bg 连续色调验证，作为答辩交付物）。
...
### 0.6 M3.0 Surface Profile Gate（2026-07-10）

M1/M2 后需要修正"写一次、多作用域复用"的解释：复用单位是**效果语义**，不是强制同一个 shader / 默认值无差别作用在文字、背景和未来 frame 上。决策见 `../../knowledge/decisions/2026-07-10-dip-fx-surface-profiles.md`。

M3 启动前先补一张 surface profile 表：

| profile 状态 | 含义 |
|---|---|
| `text-only` | 当前效果只承诺文字/文字容器语义，背景使用需另设 profile 或拆名 |
| `background-ready` | 当前实现和默认值可直接作用于背景 sprite |
| `profile-split` | 同名效果保留视觉语义，但文字与背景需要不同 shader / 默认值 / 参数解释 |
| `future-frame` | 未来可接整帧后处理，但不进入 M3 默认实现 |

M3 规则：

- 不为了"两用"回滚已经做过的纯文字适配。
- 不把 `background` / `frame` 加进 `targetType` 或 `CommandLevel`。
...
**内建实现组织**：profile-split filter 按效果语义聚合，再在效果目录内区分 surface；例如 `filters/duotone/{TextDuotoneFilter,BackgroundDuotoneFilter}`。`GrayFilter` 等通用实现保持单文件。该约定只管理内建实现，不把仓库目录暴露为未来插件 API；插件契约另见 `../ecosystem/effect-preset-and-plugin-contract.md`。
...
4. **char 守卫**：`targetType` 含 `char` 且实现里用了 `addModifier` 或假定 KineticChar 时，加 `instanceof KineticChar` 守卫并对非匹配目标 `console.warn` 后 return（参考 `warp` 在 §0.5.3 容器级扩展前的形态——扩展后 warp 不再用此守卫，改为 char/容器分支同 scanline）。
...
- **`displace`**（behavior）：位移贴图——`vTextureCoord += noise(uv*uScale + uTime) * uAmount * uInputSize.zw` 后采样。`uAmount` 是**像素值**（非 UV 每比例），`uInputSize.zw` 转 UV 偏移。噪声 `sin` 双轴组合 `clamp` 到 `[-1,1]` 中心对称（无固定偏移）。`uTime` 由 addModifier（char 级）/ ticker 回调（容器级）推进。这是 `underwater` 的几何半边。targetType both，需 padding `ceil(amount)`。
...
3. **M2**（✅ 完成）：`displace`→`underwater`（浪花镜头）、`dissolve`、`noise`、`vignette`、`scanline`。`warp` 顺带从 char-only 扩展到容器级。赛博朋克标题序列 demo 作品 `fx-cyberpunk-title.kmd` 作为答辩交付物。落地记录见 §0.5.2（vignette/scanline/noise/dissolve）+ §0.5.3（displace/underwater/warp 扩展）。
4. **M3**：非 DIP 配套 behavior 按 demo 缺口补；整理报告叙事（库边界 = DIP 定义）。
