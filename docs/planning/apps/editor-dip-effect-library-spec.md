...
5. **`displace`**（behavior）：噪声场驱动 UV 位移 `vTextureCoord += (noise(uv*uScale + uTime)*2-1) * uAmount * uInputSize.zw`。噪声用 `sin` 双轴组合生成连续平滑位移场（避免 `hash21` 白噪点的撕裂；spec §7.2 原文 `noise()` 无强制实现）。uniform `uTime`(0) / `uAmount`(0.02) / `uScale`(4) + `uInputSize`（Pixi 注入 `.zw`）。纯坐标位移，pass-through color（同 WarpFilter，不做预乘 alpha 对偶）。需 padding（位移越界）——ctor + amount setter 同步 `Math.ceil(amount * 64)`（UV 比例→像素保守估算，宁大勿小；preset 可覆盖）。char 级 `addModifier` + 容器级 `gsap.ticker.add`（模板同 scanline/noise）。`mutexGroup: "filter_displace"`。
