...
- 语料：`apps/editor/public/tests/` 的功能 fixture + 顶层 `.kmd`；纯叙事/展示作品由 `parser-golden-corpus.ts` 显式排除，交给 playback 与浏览器 E2E 覆盖，避免提交体量巨大的完整 AST 快照。
...
- 解析功能语料（`public/tests/*.kmd`，排除显式列出的展示作品）+ 顶层 `*.kmd` → **稳定规范化序列化**（`ParagraphAst` + `ParagraphIR` + tokens/effects/layoutInstructions，键序稳定；位置 range 一并保留，因 B0.1 也应保持）→ 对比**提交的黄金文件**。
...
2. 新增三类定向测试（先做，纯新增不动旧脚本）：
   (a) parser 黄金 fixture：解析功能语料（apps/editor/public/tests/*.kmd，排除显式列出的展示作品）+ 顶层 *.kmd → 稳定规范化序列化（ParagraphAst + ParagraphIR + tokens/effects/layoutInstructions，键序稳定，位置 range 保留）→ 对比提交的黄金文件。B0.1 行为中性 ⟺ 黄金零变化。审计语料是否覆盖 B0.1 触及语法（成员解析 / 量词 / cam.* / hold-ease 词形 / :bg / 括号组），缺则补 fixture。黄金更新必须人工审，禁止无脑 --update。
