...
> Phase R 已完成包边界建立；Phase B 语言设计收敛审查已完成（2026-07-08）。当前剩余 gate 是 Android 真机/模拟器 smoke 与 package artifact 消费链路稳定；满足后按重写版 `phase-b/1.6-phase-b-plan.md` 恢复 Phase B 实施（从 B0.1 新链解析器开始）。
...
- Android 真机/模拟器 WebView smoke 与 reader artifact 消费链路补实。2026-06-17 模拟器 smoke 已确认：真实 `reader-runtime-web` bundle 打进 APK `assets/reader-runtime/`、本地 HTTPS 虚拟域名加载、`runtimeReady → loadScript → ready → play → progress → ended` 全链路闭合。详见 `apps/android-reader/docs/planning/stages/stage-5-integration-test-report.md`。
- 下一道 Android 侧缺口是 runtime `ready` 首帧与 `ended` 结束态体验（smoke 暴露的黑屏观感，BUG-09），以及 renderer 错误恢复收束；详见 `apps/android-reader/docs/planning/roadmap.md`。
