...
```text
apps/editor/             Vue editor shell and editor-only integrations
apps/android-reader/     Optional local checkout of the Android app; ignored by this repo
packages/core/           Private shared parser/layout/effect/stage runtime package
...
The shared parser/layout/effect/stage implementation now lives in the private workspace package `@kmd/core`. Both the editor and `@kmd/reader-runtime-web` consume it through package imports; editor-only Monaco/TextMate integration remains under `apps/editor/`. This boundary is intentionally internal: before Phase B settles the language and runtime design, `@kmd/core` does not promise a stable public API or npm publication.
...
KMD is under active exploration. APIs, syntax details, and runtime internals may change quickly. The repository now has a private shared runtime boundary at `@kmd/core` and a hostable reader artifact at `@kmd/reader-runtime-web`; Phase B language changes remain gated by language design review and integration stability.
